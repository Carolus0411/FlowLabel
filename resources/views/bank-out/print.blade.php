...
<body>
    <div class="container">
        ...
        <div class="info-section">
            <div class="info-left">
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $bankOut->code }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Contact :</span>
                    <span class="info-value">{{ $bankOut->contact->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Supplier :</span>
                    <span class="info-value">{{ $bankOut->supplier->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Date :</span>
                    <span class="info-value">{{ \Carbon\Carbon::parse($bankOut->date)->format('d F Y') }}</span>
                </div>
            </div>
            ...
            <div class="info-right">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $bankOut->bankAccount->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Status :</span>
                    <span class="info-value">{{ ucfirst($bankOut->status) }}</span>
                </div>
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                @forelse($bankOut->details as $index => $detail)
                <tr>
                ...
                @endforelse
                @if($bankOut->details->count() > 0)
                <tr class="total-row">
                    <td colspan="6" class="text-right">Total Amount (IDR) :</td>
                    <td class="text-right">{{ number_format($bankOut->total_amount, 2, ',', '.') }}</td>
                </tr>
                @endif
            </tbody>
        </table>
        ...
        <div class="signatures">
            <div class="signature-box">
                ...
                <div class="signature-name">{{ $bankOut->contact->name ?? $bankOut->supplier->name ?? 'Recipient' }}</div>
            </div>
