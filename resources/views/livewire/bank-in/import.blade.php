...
new class extends Component {
    ...
    public function save()
    {
        ...
        try {
            ...
            DB::commit();
            $this->success('Success','Bank successfully imported.', redirectTo: route('bank-in.index'));
        }
        catch (Exception $e)
        {
            DB::rollBack();
            logger()->error($e->getMessage());
            $this->error('Error','Bank failed to import.', redirectTo: route('bank-in.index'));
        }
    }
}; ?>

<div>
    <x-header title="Import Bank In" separator />
    <div
    ...
    >
        <x-card>
            <x-form wire:submit="save">
                ...
                <x-slot:actions>
                    <x-button label="Cancel" link="{{ route('bank-in.index') }}" wire:target="save" wire:loading.attr="disabled" />
