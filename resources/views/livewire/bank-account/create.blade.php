...
new class extends Component {
    ...
    {
        Gate::authorize('create bank-account');
        $this->searchBank();
        $this->searchCurrency();
        $this->searchCoa();
    }
    ...
    {
        $data = $this->validate([
            'code' => 'required|unique:App\Models\BankAccount,code',
            'name' => 'required',
            'bank_id' => 'required',
        ...
        $this->success('Bank account successfully created.', redirectTo: route('bank-account.index'));
    }
    ...
    {
        $selected = Coa::where('id', $this->coa_code)->get();
        $this->coas = Coa::query()
            ->filterLike('name', $value)
            ->isActive()
            ->take(20)
...
<div>
    <x-header title="Create Bank Account" separator>
        <x-slot:actions>
