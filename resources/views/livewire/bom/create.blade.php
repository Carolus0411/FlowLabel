...
new #[Layout('components.layouts.app')] class extends Component {
    ...
    {
        $this->bom_no = BOM::generateBOMNo();
        $this->bom_date = date('Y-m-d');
        $this->addDetail();
    }
    ...
    {
        $this->validate([
            'bom_no' => 'required|unique:boms,bom_no',
        ...
        try {
            ...
            $this->success('BOM created successfully');
            return redirect()->route('bom.show', $bom->id);
        } catch (\Exception $e) {
            $this->error('Failed to create BOM: ' . $e->getMessage());
        }
    }
}; ?>

<div>
    <x-header title="Create Bill Of Material" separator />

    <x-form wire:submit="save">
        <x-card title="BOM Information">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <x-input label="BOM No" wire:model="bom_no" readonly required />
        ...
        <div class="flex justify-end gap-4 mt-4">
            <x-button label="Cancel" link="{{ route('bom.index') }}" />
            <x-button label="Save BOM" type="submit" class="btn-primary" />
        </div>
    </x-form>
</div>
