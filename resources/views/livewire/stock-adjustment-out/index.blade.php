...
new class extends Component {
    ...
    #[Session(key: 'stock_adjustment_out_per_page')]
    public int $perPage = 10;

    #[Session(key: 'stock_adjustment_out_date1')]
    public string $date1 = '';

    #[Session(key: 'stock_adjustment_out_date2')]
    public string $date2 = '';

    #[Session(key: 'stock_adjustment_out_code')]
    public $code = '';

    #[Session(key: 'stock_adjustment_out_status')]
    public $status = '';
    ...
    {
        Gate::authorize('view stock adjustment out');
    ...
    public function headers(): array
    {
        return [
            ...
            ['key' => 'code', 'label' => 'No. SAO', 'class' => 'truncate'],
    ...
    {
        $adjustment = StockAdjustmentOut::create([
            'code' => uniqid('SAO/'),
            'date' => now(),
            'status' => 'open',
        ]);

        $this->redirectRoute('stock-adjustment-out.edit', $adjustment->id);
    }

    public function adjustments(): LengthAwarePaginator
    {
        return StockAdjustmentOut::stored()
...
<div>
    ...
    <x-card>
        <x-table
            ...
            link="/stock-adjustment-out/{id}/edit"
        >
            ...
            @scope('cell_act', $adjustment)
                <x-dropdown>
                    ...
                    @can('update stock adjustment out')
                    <x-menu-item title="Edit" icon="o-pencil" link="{{ route('stock-adjustment-out.edit', $adjustment->id) }}" />
                    @endcan
                    @if($adjustment->status == 'open' && Gate::allows('approve stock adjustment out'))
                    <x-menu-item title="Approve" icon="o-check" link="{{ route('stock-adjustment-out.edit', $adjustment->id) }}" />
                    @endif
                    @if($adjustment->status == 'close' && Gate::allows('void stock adjustment out'))
                    <x-menu-item title="Void" icon="o-x-mark" link="{{ route('stock-adjustment-out.edit', $adjustment->id) }}" />
                    @endif
                </x-dropdown>
            @endscope
