...
new class extends Component {
    ...
    {
        $this->purchaseReceival = PurchaseReceival::find($id);
    ...
    {
        $this->open = $this->purchaseReceival->status == 'open' && $this->editable;

        $details = $this->purchaseReceival->details()
    ...
    public function headers(): array
    {
        return [
            ...
            ['key' => 'received_qty', 'label' => 'Prev Received', 'class' => 'text-right', 'format' => ['currency', '2.,', '']],
    ...
    public function edit(PurchaseReceivalDetail $detail): void
    {
        if (!$this->open) return;
    ...
    public function delete(PurchaseReceivalDetail $detail): void
    {
        if (!$this->open) return;
    ...
    {
        $totalQty = $this->purchaseReceival->details()->sum('qty');
        $totalAmount = $this->purchaseReceival->details()->sum('amount');
...
<div>
    ...
    <x-drawer wire:model="drawer" :title="$mode == 'add' ? 'Add Detail' : 'Edit Detail'" right separator with-close-button class="lg:w-1/3">
        <div class="space-y-4">
            ...
            <x-input label="Previously Received" wire:model="received_qty" readonly class="bg-base-200" />
            <x-input label="Qty to Receive" wire:model="qty" x-mask:dynamic="$money($input, '.', ',')" />
