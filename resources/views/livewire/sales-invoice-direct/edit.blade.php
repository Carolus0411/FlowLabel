...
new class extends Component {
    ...
    public SalesInvoiceDirect $salesInvoiceDirect;
    ...
    public function mount(SalesInvoiceDirect $salesInvoiceDirect): void
    {
        Gate::authorize('update sales-invoice-direct');
        $this->salesInvoiceDirect = $salesInvoiceDirect;
        $this->fill($this->salesInvoiceDirect);
        $this->searchContact();
        $this->searchPpn();
        $this->searchPph();
        $this->calculate();
    ...
    {
        $this->open = $this->salesInvoiceDirect->status == 'open';
        ...
        if (($this->invoice_type ?? $this->salesInvoiceDirect->invoice_type ?? '') === 'AP') {
    ...
    public function save($close = false): void
    {
        ...
        $data = $this->validate([
            ...
            // 'details' => new \App\Rules\SalesOrderDetailCheck($this->salesInvoiceDirect), // Need to check if this rule works for Direct or needs duplication
        ]);

        unset($data['details']);

        if ($this->salesInvoiceDirect->saved == '0') {
        ...
        $data['balance_amount'] = Cast::number($this->invoice_amount);

        $this->salesInvoiceDirect->update($data);

        if ($close) {
            $this->close();
        }

        $this->success('Invoice successfully updated.', redirectTo: route('sales-invoice-direct.index'));
    }
    ...
    public function delete(SalesInvoiceDirect $salesInvoiceDirect): void
    {
        ...
        $salesInvoiceDirect->details()->delete();
        $salesInvoiceDirect->delete();
        $this->success('Invoice successfully deleted.', redirectTo: route('sales-invoice-direct.index'));
    }

    public function void(SalesInvoiceDirect $salesInvoiceDirect): void
    {
        Gate::authorize('void sales-invoice-direct');
        $salesInvoiceDirect->update([
            'status' => 'void'
        ]);

        \App\Events\SalesInvoiceDirectVoided::dispatch($this->salesInvoiceDirect);

        $this->success('Invoice successfully voided.', redirectTo: route('sales-invoice-direct.index'));
    }

    public function close(): void
    {
        Gate::authorize('close sales-invoice-direct');
        \App\Events\SalesInvoiceDirectClosed::dispatch($this->salesInvoiceDirect);
    }
}; ?>
        ...
        <x-header separator>
            <x-slot:title>
                <div class="flex items-center gap-4">
                    <span>Update Sales Invoice Direct</span>
                    <x-status-badge :status="$salesInvoiceDirect->status" class="uppercase !text-sm" />
                    @if ($salesInvoiceDirect->status == 'close')
                    <x-payment-status-badge :data="$salesInvoiceDirect" class="uppercase !text-sm" />
                    @endif
                </div>
            </x-slot:title>
            <x-slot:actions>
                <x-button label="Back" link="{{ route('sales-invoice-direct.index') }}" icon="o-arrow-uturn-left" class="btn-soft" responsive />

                @if ($salesInvoiceDirect->status == 'close')
                <x-button label="Journal" icon="o-document-text" class="btn-accent" onclick="popupWindow('{{ route('print.journal', ['SalesInvoiceDirect', base64_encode($salesInvoiceDirect->code)]) }}', 'journal', '1000', '460', 'yes', 'center')" responsive />
                @endif
                @if ( $salesInvoiceDirect->status == 'open')
    ...
    <div class="space-y-4">
        <x-card>
            <x-form wire:submit="save">
                ...
                {{-- <x-slot:actions>
                    <x-button label="Cancel" link="{{ route('sales-invoice-direct.index') }}" />
        ...
        <div class="overflow-x-auto">
            <livewire:sales-invoice-direct.detail
                :id="$salesInvoiceDirect->id"
                :transport="$transport"
                :service_type="$service_type"
            />
        </div>

        @if ($salesInvoiceDirect->saved == '1')
        <div class="space-y-4 lg:space-y-0 lg:grid grid-cols-2 gap-4">
            <x-card>
                <div class="space-y-4">
                    <x-other-info :data="$salesInvoiceDirect" />

                    <h2 class="text-lg font-semibold">Danger Zone</h2>
                    @can('void sales-invoice-direct')
                    @if ($salesInvoiceDirect->status != 'void')
                    <div class="text-xs">
                    ...
                    <div>
                        <x-button
                            ...
                            wire:click="void('{{ $salesInvoiceDirect->id }}')"
                            spinner="void('{{ $salesInvoiceDirect->id }}')"
                    ...
                    @if ($salesInvoiceDirect->status == 'void')
                    @can('delete sales-invoice-direct')
                    <div class="text-xs">
                    ...
                    <div>
                        <x-button
                            ...
                            icon="o-trash"
                            wire:click="delete('{{ $salesInvoiceDirect->id }}')"
                            spinner="delete('{{ $salesInvoiceDirect->id }}')"
            ...
            <x-logs :data="$salesInvoiceDirect" />
        </div>
        @endif
