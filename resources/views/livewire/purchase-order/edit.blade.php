...
new class extends Component {
    ...
    {
        Gate::authorize('update purchase-order');

        $this->purchaseOrder = $purchaseOrder;

        // Debug: Check if purchaseOrder is loaded
        if (!$this->purchaseOrder || !$this->purchaseOrder->exists) {
            $this->error('Purchase order not found');
            return;
        }

        $this->fill($this->purchaseOrder);
        $this->searchSupplier();
        $this->searchPpn();
        $this->searchPph();
    }
    ...
    public function save(): void
    {
        $data = $this->validate([
            ...
            'note' => 'nullable',
            'supplier_id' => 'nullable', // Allow empty for existing records
            'top' => 'nullable|integer|gt:0', // Allow empty for existing records
            'ppn_id' => 'nullable', // Allow empty for existing records
            'pph_id' => 'nullable', // Allow empty for existing records
            'stamp_amount' => 'nullable',
        ]);

        $this->calculate();

        // Generate proper code if this is still a draft (not saved)
        if ($this->purchaseOrder->saved == 0 || $this->purchaseOrder->saved == '0') {
        ...
        $data['saved'] = 1; // Ensure record remains saved
        $data['updated_by'] = auth()->user()->id ?? 1;

        try {
            $this->purchaseOrder->update($data);
            $this->success('Order successfully updated.', redirectTo: route('purchase-order.index'));
        } catch (\Exception $e) {
    ...
    {
        $this->purchaseOrder->update(['status' => 'void']);
        $this->success('Order successfully voided.', redirectTo: route('purchase-order.index'));
    }

    public function close(): void
    {
        Gate::authorize('close purchase-order');
        $this->purchaseOrder->update(['status' => 'close']);
        $this->closeConfirm = false;
        $this->success('Order successfully approved.', redirectTo: route('purchase-order.index'));
    }

    public function delete(): void
    {
        $this->purchaseOrder->delete();
        $this->success('Order successfully deleted.', redirectTo: route('purchase-order.index'));
    }
...
<div>
        ...
        <x-header separator>
            <x-slot:title>
                <div class="flex items-center gap-4">
                    ...
                    <x-status-badge :status="$purchaseOrder->status" class="uppercase text-sm!" />
                </div>
            </x-slot:title>
            <x-slot:actions>
                <x-button label="Back" link="{{ route('purchase-order.index') }}" icon="o-arrow-uturn-left" class="btn-soft" responsive />
    ...
    <div class="space-y-4">
        <x-card>
            <div class="space-y-4">
                <div class="space-y-4 lg:space-y-0 lg:grid grid-cols-3 gap-4">
                    ...
                    <x-select label="Order Type" wire:model="invoice_type" :options="[['id' => 'PO','name' => 'PO']]" placeholder="-- Select --" />
                    <x-choices
                        label="Supplier"
        ...
        <div class="overflow-x-auto">
            <livewire:purchase-order.detail
                :id="$purchaseOrder->id ?? 'new'"
            />
        </div>
    </div>
