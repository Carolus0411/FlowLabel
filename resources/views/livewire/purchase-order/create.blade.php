...
new class extends Component {
    ...
    {
        Gate::authorize('create purchase-order');
        $this->order_date = date('Y-m-d');
        $this->due_date = date('Y-m-d');
        $this->searchSupplier();
        $this->searchPpn();
        $this->searchPph();
        $this->invoice_type = 'AP';
    }
    ...
    #[On('detail-updated')]
    public function detailUpdated(array $data = []): void
    {
    ...
    public function save($close = false): void
    {
        ...
        $this->success('Order successfully created.', redirectTo: route('purchase-order.edit', $purchaseOrder->id));
    }
            ...
            <x-form wire:submit="save">
                <div class="space-y-4">
                    <div class="space-y-4 lg:space-y-0 lg:grid grid-cols-3 gap-4">
                        ...
                        <x-select label="Order Type" wire:model="invoice_type" :options="[['id' => 'OP','name' => 'OP']]" placeholder="-- Select --" />
                        <x-choices
                            label="Supplier"
