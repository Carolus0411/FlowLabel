...
new class extends Component {
    ...
    {
        Gate::authorize('create other-payable-invoice');
        $this->invoice_date = date('Y-m-d');
        $this->due_date = date('Y-m-d');
        $this->searchSupplier();
        $this->searchPpn();
        $this->searchPph();
        $this->invoice_type = 'OP';
    }
    ...
    public function save($close = false): void
    {
        ...
        $invoice = OtherPayableInvoice::create($data);

        if ($close) {
            $invoice->update(['status' => 'close']);
        }

        $this->success('Invoice successfully created.', redirectTo: route('other-payable-invoice.edit', $invoice->id));
    }
        ...
        <x-header separator>
            <x-slot:title>
                <div class="flex items-center gap-4">
                    <span>Create Other Payable Invoice</span>
                </div>
            </x-slot:title>
            <x-slot:actions>
            ...
            <x-form wire:submit="save">
                <div class="space-y-4">
                    <div class="space-y-4 lg:space-y-0 lg:grid grid-cols-3 gap-4">
                        ...
                        <x-select label="Invoice Type" wire:model="invoice_type" :options="[['id' => 'OP','name' => 'OP']]" placeholder="-- Select --" />
                        <x-choices
                            label="Supplier"
