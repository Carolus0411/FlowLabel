...
new class extends Component {
    ...
    #[Session(key: 'apoutstanding_date')]
    public string $date = '';

    #[Session(key: 'apoutstanding_supplier_id')]
    public $supplier_id = '';

    #[Session(key: 'apoutstanding_code')]
    public $code = '';
    ...
    #[Session(key: 'apoutstanding_per_page')]
    ...
    public function mount(): void
    {
        ...
        // Initialize suppliers list for the Supplier filter
        $this->searchSupplier('');

        $this->updateFilterCount();
    }

    public function apInvoices(): LengthAwarePaginator
    {
        return $this->apQuery()->paginate($this->perPage);
    }

    public function apQuery()
    {
        $query = PurchaseInvoice::stored()
            ...
            ->with(['supplier','settlementDetails.purchaseSettlement'])
            ->orderBy('id','asc');

        return $query;
    }

    public function apTotals(): array
    {
        $query = $this->apQuery()->toBase();
    ...
    public function headers(): array
    {
        return [
            ...
            ['key' => 'supplier.name', 'label' => 'Supplier', 'sortable' => false, 'class' => 'truncate max-w-[200px]'],
    ...
    public function export()
    {
        ...
        $query = $this->apQuery()->orderBy('id','asc');
        $writer = SimpleExcelWriter::streamDownload('AP Outstanding.xlsx');
        foreach ($query->lazy() as $invoice) {
            $settlements = '';
            if ($invoice->settlementDetails->isNotEmpty()) {
                $settlements = $invoice->settlementDetails->map(function($d){
                    return $d->purchase_settlement_code . ' (' . ($d->purchaseSettlement->date ?? '') . ') : ' . number_format($d->amount ?? 0,2);
                })->implode('; ');
            }
...
<div>
    <x-header title="AP Outstanding" separator>
        <x-slot:subtitle>
            Show unpaid and outstanding supplier invoices with applied settlements
        </x-slot:subtitle>
        <x-slot:actions>
    ...
    <x-card wire:loading.class="bg-slate-200/50 text-slate-400">
        <x-table :headers="$headers" :rows="$apInvoices" :sort-by="$sortBy" with-pagination per-page="perPage" show-empty-text>
            ...
            <ul class="list-disc pl-4 text-sm">
                @foreach ($invoice->settlementDetails as $detail)
                    <li>
                        <strong>{{ $detail->purchase_settlement_code }}</strong> (
                        {{ $detail->purchaseSettlement->date ?? '' }}) : {{ number_format($detail->amount ?? 0, 2) }}
                    </li>
                @endforeach
            </ul>
            @else
