...
new class extends Component {
    ...
    {
        Gate::authorize('update purchase-invoice-direct');
    ...
    public function save($close = false): void
    {
        ...
        $this->success('Invoice successfully updated.', redirectTo: route('purchase-invoice-direct.index'));
    }
    ...
    {
        Gate::authorize('delete purchase-invoice-direct');
        $purchaseInvoice->details()->delete();
        $purchaseInvoice->delete();
        $this->success('Invoice successfully deleted.', redirectTo: route('purchase-invoice-direct.index'));
    }

    public function void(PurchaseInvoice $purchaseInvoice): void
    {
        Gate::authorize('void purchase-invoice-direct');
        $purchaseInvoice->update(['status' => 'void']);
        $this->success('Invoice successfully voided.', redirectTo: route('purchase-invoice-direct.index'));
    }

    public function close(): void
    {
        Gate::authorize('close purchase-invoice-direct');
        \App\Events\PurchaseInvoiceDirectClosed::dispatch($this->purchaseInvoice);
    }
}; ?>
        ...
        <x-header separator>
            <x-slot:title>
                <div class="flex items-center gap-4">
                    <span>Update Purchase Invoice Direct</span>
            ...
            <x-form wire:submit="save">
                <div class="space-y-4">
                    <div class="space-y-4 lg:space-y-0 lg:grid grid-cols-3 gap-4">
                        ...
                        <x-select label="Invoice Type" wire:model="invoice_type" :options="[['id' => 'APD','name' => 'APD']]" placeholder="-- Select --" :disabled="!$open" />
                        <x-choices
                            label="Supplier"
        ...
        <div class="space-y-4 lg:space-y-0 lg:grid grid-cols-2 gap-4">
            <x-card>
                <div class="space-y-4">
                    ...
                    @can('void purchase-invoice-direct')
                    ...
                    @if ($purchaseInvoice->status == 'void')
                    @can('delete purchase-invoice-direct')
                    <div class="text-xs">
