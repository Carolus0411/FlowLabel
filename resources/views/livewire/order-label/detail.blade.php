...
new class extends Component {
    ...
    {
        if ($id !== 'new') {
            $this->salesOrder = SalesOrder::with(['contact', 'ppn', 'pph'])->find($id);
        } else {
            // Create a temporary instance for new orders
    ...
    {
        $this->open = is_null($this->salesOrder->id) || $this->salesOrder->status == 'open';

        $details = [];
        if (Schema::hasTable('sales_order') && !is_null($this->salesOrder->id)) {
            $details = $this->salesOrder->details()->with(['serviceCharge','currency','uom'])->get();
        }
    ...
    public function edit(SalesOrderDetail $detail): void
    {
        $this->clearForm();
    ...
    {
        // Cannot save details for unsaved order
        if (is_null($this->salesOrder->id)) {
            $this->error('Please save the order first before adding details.');
            return;
        }
        ...
        if ($this->mode == 'add')
        {
            $this->salesOrder->details()->create([
    ...
    public function calculate(): void
    {
        ...
        $dpp_amount = $this->salesOrder->details()->sum('amount');
