...
<body>
    <div class="container">
        ...
        <div class="info-section">
            <div class="info-left">
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $cashIn->code }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Contact :</span>
                    <span class="info-value">{{ $cashIn->contact->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Date :</span>
                    <span class="info-value">{{ \Carbon\Carbon::parse($cashIn->date)->format('d F Y') }}</span>
                </div>
            </div>
            ...
            <div class="info-right">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $cashIn->cashAccount->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Status :</span>
                    <span class="info-value">{{ ucfirst($cashIn->status) }}</span>
                </div>
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                @forelse($cashIn->details as $index => $detail)
                <tr>
                ...
                @endforelse
                @if($cashIn->details->count() > 0)
                <tr class="total-row">
                    <td colspan="6" class="text-right">Total Amount (IDR) :</td>
                    <td class="text-right">{{ number_format($cashIn->total_amount, 2, ',', '.') }}</td>
                </tr>
                @endif
            </tbody>
        </table>
        ...
        <div class="signatures">
            <div class="signature-box">
                ...
                <div class="signature-name">{{ $cashIn->contact->name ?? 'Contact' }}</div>
            </div>
