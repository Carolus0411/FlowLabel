...
<head>
    ...
    <title>Other Payable Settlement</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        ...
        <div class="title">
            <h1>OTHER PAYABLE SETTLEMENT</h1>
        </div>
        ...
        <div class="info-section">
            <div class="info-left">
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $otherPayableSettlement->code }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Supplier :</span>
                    <span class="info-value">{{ $otherPayableSettlement->supplier->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Date :</span>
                    <span class="info-value">{{ \Carbon\Carbon::parse($otherPayableSettlement->date)->format('d F Y') }}</span>
                </div>
            </div>
            ...
            <div class="info-right">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $otherPayableSettlement->bankAccount->name ?? 'N/A' }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label">Status :</span>
                    <span class="info-value">{{ ucfirst($otherPayableSettlement->status) }}</span>
                </div>
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                @forelse($otherPayableSettlement->details as $index => $detail)
                <tr>
                ...
                @endforelse
                @if($otherPayableSettlement->details->count() > 0)
                <tr class="total-row">
                    <td colspan="6" class="text-right">Total Amount (IDR) :</td>
                    <td class="text-right">{{ number_format($otherPayableSettlement->total_amount, 2, ',', '.') }}</td>
                </tr>
                @endif
            </tbody>
        </table>

        @if($otherPayableSettlement->note)
        <div style="margin-bottom: 30px;">
            <strong>Note:</strong><br>
            {{ $otherPayableSettlement->note }}
        </div>
        @endif
        ...
        <div class="signatures">
            <div class="signature-box">
                ...
                <div class="signature-name">{{ $otherPayableSettlement->supplier->name ?? 'Supplier' }}</div>
            </div>
